...
UTI.pdp = {
    init: function () {
        ...
        $('.product-set-'+1).find('.gallery-inside').flickity({
             "pageDots": true,"contain": true,"arrows":false
        });
        ...
        function resizedw(){
            if(wid != $(window).width())
            {
                $('.product-set-'+galenum).find('.gallery-inside.flickity-enabled').flickity('destroy');
                $('.product-set-'+galenum).find('.gallery-inside').flickity({
                    "pageDots": true,"contain": true
                });
                checkscale();
                wid = $(window).width();
            }
...
PROJ.ajaxcart = {
    ...
    init: function () {
        ...
        $('body').on('click', '[data-vardirectadd]', function (e) {
            e.preventDefault();
            $(this).parents('.checkadd').find('[data-directadd]').attr('data-directadd', $(this).attr('data-vardirectadd')).show();
    ...
    updateCart: function () {
        ...
        $.get("/cart?view=mini", function (data) {
            ...
            if($('[data-total-price]').attr('data-total-price'))
            {
                ...
                $('.txt.totalcart').html('<b>'+$('[data-total-price]').attr('data-total-item')+ itsmc+"</b>"+$('[data-total-price]').attr('data-total-price'));
            }else
            {
                $('.txt.totalcart').text('');
            }
...
UTI.effect = {
    ...
    playlogo: function () {
        ...
        var logostart = gsap.timeline({
            paused: true, defaults: { duration: 2 }
        });
